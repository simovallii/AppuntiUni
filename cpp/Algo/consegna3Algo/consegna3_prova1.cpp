...
int DFS(int n, int* seq, int seq_size, int pos) {
    ...
    while (elem != NULL) {
        ...
        if (DFS(elem->val, seq, seq_size, pos + 1))
            return 1;
...
int main(int argc, char **argv) {
    ...
    // Definizione sequenza da cercare
    int seq_target[] = {0, 2, 4}; 
    int n_target = 3;
    int esito = 0;

    // Provo a far partire il cammino da ogni nodo del grafo
    for (int i = 0; i < n_nodi; i++) {
        if (DFS(i, seq_target, n_target, 0)) {
            esito = 1;
            break;
        }
    }

    cout << "Cammino trovato? " << (esito ? "SI" : "NO") << endl;
