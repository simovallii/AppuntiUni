...
//       - Prefix sum          : BUCKETS letture di count  (256 iterazioni)
//       - Scatter su A[]      : n       letture di A
//       - Scatter su count[]  : n       letture di count  (read-modify-write)
//     Per passaggio: 2n + 2n + BUCKETS = 4*1000 + 256 = 4256
//     Totale 2 passaggi: 2 * 4256 = 8512 letture
//
//   Quicksort baseline: ~32146 letture medie => miglioramento ~3.8x
...
void radix_sort(int *A, int n_elem) {
    ...
    for (int pass = 0; pass < PASSES; pass++) {
        ...
        // Si accede solo a count[], che e' in memoria: ogni count[i] e count[i-1]
        // e' una lettura. Sono BUCKETS-1 iterazioni, ognuna legge count[i] e count[i-1].
        // Contiamo 1 lettura per iterazione (count[i-1] e' gia' stato letto/scritto prima,
        // count[i] e' il target della scrittura — la lettura rilevante e' count[i-1]).
